/*
  ...
  - You are about to drop the column `itemsId` on the `sales` table. All the data in the column will be lost.
  ...
  - Added the required column `itemId` to the `sales` table without a default value. This is not possible if the table is not empty.

*/
-- DropForeignKey
ALTER TABLE `sales` DROP FOREIGN KEY `sales_itemsId_fkey`;
...
-- AlterTable
ALTER TABLE `sales` DROP COLUMN `itemsId`,
...
-- AddForeignKey
ALTER TABLE `sales` ADD CONSTRAINT `sales_itemId_fkey` FOREIGN KEY (`itemId`) REFERENCES `purchaseitems`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
