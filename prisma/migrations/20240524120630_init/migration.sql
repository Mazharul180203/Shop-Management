/*
  ...
  - You are about to drop the column `payemt_type` on the `supplierledger` table. All the data in the column will be lost.
  ...
  - Added the required column `payment_type` to the `supplierledger` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `supplierledger` DROP COLUMN `payemt_type`,
